...
class RepliesService: Service {
    ...
    func getCounts(completion: @escaping(Result<GetUnreadCountResponse, Error>) -> Void) {
        requestHandler.makeApiRequest(host: SessionStorage.getInstance.getLemmyInstance(), request: "/user/unread_count", method: .get) { result in
            self.respond(result, completion)
        }
    }
    ...
    func markAsRead(commentReply: CommentReply, read: Bool, completion: @escaping(Result<CommentReplyResponse, Error>) -> Void) {
        ...
        requestHandler.makeApiRequest(host: SessionStorage.getInstance.getLemmyInstance(), request: "/comment/mark_as_read", method: .post, body: body) { result in
            self.respond(result, completion)
        }
    }
    
    func markAllAsRead(completion: @escaping(Result<GetRepliesResponse, Error>) -> Void) {
        requestHandler.makeApiRequest(host: SessionStorage.getInstance.getLemmyInstance(), request: "/user/mark_all_as_read", method: .post) { result in
            self.respond(result, completion)
        }
    }
}
